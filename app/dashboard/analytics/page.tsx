...
import {Container, Grid, PaperProps, rem, SimpleGrid, Skeleton, Stack, useMantineTheme} from "@mantine/core";
import {
  ErrorAlert,
  LanguageTable,
  MapChart,
...
function Page() {
  ...
  const {data: languagesData, error: languageError, loading: languageLoading} = useFetchData("/mocks/Languages.json")
  const {data: trafficData, error: trafficError, loading: trafficLoading} = useFetchData("/mocks/Traffic.json")
    ...
    <>
      ...
      <Container fluid>
        <Stack gap="lg">
          ...
          >
            {statsError ?
              <ErrorAlert title="Error loading stats" message={statsError.toString()}/> :
              <SimpleGrid cols={2}>
                {statsLoading ?
