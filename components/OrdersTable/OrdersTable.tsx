...
const OrdersTable = ({data}: OrdersTableProps) => {
    ...
    const [sortStatus, setSortStatus] = useState<DataTableSortStatus>({columnAccessor: 'product', direction: 'asc'});
    ...
    useEffect(() => {
        ...
        if (debouncedQuery || selectedStatuses.length) {
            filtered =
                data.filter(({product, status}) => {
                    if (debouncedQuery !== '' && !product.toLowerCase().includes(debouncedQuery.trim().toLowerCase())) {
                        return false;
                    }
